...
struct allocator {
  ...
  // - p 必须是通过先前对 allocate() 或 allocate_at_least()(C++23 起) 的调用所获得的指针
  // - n 必须等于最初产生 p 的 allocate() 调用的第一个参数
  //   或者如果 p 是从返回 {p, count} 的 allocate_at_least(m) 调用获得的，则在范围 [m, count] 内(C++23 起)
  // - 调用 ::operator delete(void*) 或 ::operator delete(void*, std::align_val_t)(C++17 起)
  ...
  constexpr void deallocate(T* p, size_type n) noexcept {
    ...
    // 如果对齐要求超过默认对齐，使用对齐的 operator delete（C++17）
  ...
  // 选择容器复制构造时的分配器（C++11）
  // 根据 cppreference.com/std::allocator/select_on_container_copy_construction
  constexpr allocator select_on_container_copy_construction() const {
    return *this;
  }
  
  // rebind 成员模板类（用于获取不同类型的分配器）
  // 注意：C++11 前使用 rebind<U>::other，C++11 起推荐使用 allocator_traits<A>::rebind_alloc<U>
  // 保留以保持向后兼容性
  template <class U>
  struct rebind {
...
// 注意：operator== 和 operator!= 在 C++20 中已移除
// C++20 前存在，C++20 起已移除（使用 allocator_traits 进行比较）
// 为了符合 C++23 标准，不提供这些运算符
